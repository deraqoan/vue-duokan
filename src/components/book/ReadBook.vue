<template>
	<div id="container" class="container">
		<v-touch class="content" @tap="operationAction($event)" :class="{'night-mode':nightMode}">
			<br>
      <h3>{{$store.state.bookInfo.title}}</h3>
      <h4>{{bookChaptersContent.title}}</h4>
			<article v-html="bookChaptersBody"></article>
      <br>
		</v-touch>
    <div class="menuf" :class="{'night-btn':nightMode}">
      <router-link to="/" class="menu-btn">
        <img src="../../assets/home.png">
      </router-link>
      <router-link :to="/book/+$store.state.bookInfo._id" class="menu-btn">
      ...
      <v-touch class="menu-btn" @tap="showChapter">
		...
		<div class="menu" :class="{'night-btn':nightMode}">
			...
			<v-touch class="menu-btn" @tap="upChapter">
				<img src="../../assets/on.png">
			</v-touch>
      <v-touch class="menu-btn" @tap="showChapter">
        <img src="../../assets/list.png">
      </v-touch>
      <v-touch class="menu-btn" @tap="downChapter">
...
export default {
  ...
  created () {
    ...
    api.getChapters(this.$store.state.source).then(response => {
      ...
      this.currentChapter = readRecord && readRecord[this.$route.params.bookId] && readRecord[this.$route.params.bookId].chapter ? this.$store.state.chapter : 0
      // 默认取前50章节
    ...
    getBookChapterContent () {
      ...
      api.getBookChapterContent(this.bookChapter.chapters[lastChapter].link).then(response => {
        ...
        if (response.data.chapter.cpContent && response.data.chapter.isVip) {
          MessageBox.alert('章节为正版源，请换源后重试')
        } else {
...
.container {
	...
	overflow: auto;
	color: #000;
  font-family: "Microsoft Yahei", 微软雅黑, STHeiti, Hei, "Heiti SC", 黑体;
}
